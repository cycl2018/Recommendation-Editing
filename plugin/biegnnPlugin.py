...
class biegnnPlugin(nn.Module):
    def __init__(self,model,sparse_norm_adj) -> None:
        ...
        self.user_editor = mlp(dim,dim,dim) 
        self.item_editor = mlp(dim,dim,dim) 
    
    def forward(self,*args):
        user_emb = self.user_emb + self.user_editor(self.user_emb)
        item_emb = self.item_emb + self.item_editor(self.item_emb)
