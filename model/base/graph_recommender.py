...
class GraphRecommender(Recommender):
    ...
    def fast_evaluation(self, epoch):
        ...
        if len(self.bestPerformance) > 0:
            ...
            if count < 0:
                ...
                if not os.path.exists(out_dir):
                    os.makedirs(out_dir)
                file_name = self.config['model.name'] + '@best' + '.pth'
                torch.save(self.model.state_dict(),out_dir+file_name)
        else:
        ...
        if epoch <= 100:
            ...
            if not os.path.exists(out_dir):
                os.makedirs(out_dir)
            file_name = self.config['model.name'] + '@' + str(epoch) + '.pth'
            print('Save CheckPoint in :',out_dir+file_name)
            torch.save(self.model.state_dict(),out_dir+file_name) 
