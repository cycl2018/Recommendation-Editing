...
class lwfEdit(baseEdit):
    ...
    def edit(self,edit_pairs,model):
        ...
        if self.args.edit_loss == 'bce':
            org_score = (user_embs[users] * item_embs[items]).sum(dim=-1).detach().clone()
        else:
            org_pos_score = (user_embs[pos_u] * item_embs[pos_i]).sum(dim=-1).detach().clone()
            org_neg_score = (user_embs[pos_u] * item_embs[neg_i]).sum(dim=-1).detach().clone()
            org_score = torch.concat([org_pos_score,org_neg_score],dim=-1) 
        while True : 
            edit_round += 1
            ...
            if self.args.edit_loss == 'bce':
                ...
                loss_dis = F.mse_loss(org_score,score)  
            else:
                ...
                loss_dis = F.mse_loss(org_score,torch.concat([pos_score,neg_score],dim=-1))
            
            loss_edit += self.args.alpha * loss_dis  
